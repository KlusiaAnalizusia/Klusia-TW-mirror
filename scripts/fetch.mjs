...
const SOURCES = [
    ...
    {
        out: 'conquers_extended', file: 'conquer_extended.txt',
        fields: ['village', 'ts', 'newOwner', 'oldOwner', 'oldTribe', 'newTribe', 'points'],
        text: [], min: 0, allowEmpty: true, optional: true,
    },
    ...
    { out: 'kills_sup', file: 'kill_sup.txt', fields: ['rank', 'id', 'score'], text: [], min: 0, allowEmpty: true, optional: true },
];
...
async function buildWorld(world) {
    ...
    for (const source of SOURCES) {
        ...
        try {
            ...
            // Pliki oznaczone jako optional nie istnieją na części światów —
            // brak takiego pliku to normalny stan, nie awaria. Nie publikujemy go
            // wtedy wcale, żeby skrypt dostał 404 i sięgnął po wariant zapasowy.
            if (source.optional && /HTTP 404/.test(String(e.message || e))) {
                meta.files[source.out] = { rows: null, bytes: 0, stale: false, missing: true };
                console.log(`  – ${world}/${name} — brak na tym świecie`);
    ...
    const worlds = index.worlds.map(w => {
        const rows = Object.entries(w.files).map(([name, f]) => `
            <tr${f.stale ? ' class="stale"' : ''}>
              <td>${f.missing ? esc(name) : `<a href="${esc(w.world)}/${esc(name.endsWith('.xml') ? name : name + '.json')}">${esc(name)}</a>`}</td>
              <td class="num">${fmtRows(f.rows)}</td>
              <td class="num">${fmtBytes(f.bytes)}</td>
              <td class="state">${f.missing ? 'brak na tym świecie' : f.stale ? 'poprzednie dane' : 'aktualne'}</td>
            </tr>`).join('');
        return `
        <section class="world">
          <header>
